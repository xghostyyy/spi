...
| `SMTP_API_KEY` | Отправка кодов входа (HTTP API, приоритет) | smtp.bz, домен `spi-2015.ru` (см. §1.1) |
| `SMTP_*`, `MAIL_FROM` | Отправка кодов входа (сырой SMTP, fallback) | используется только если `SMTP_API_KEY` пуст (см. §1.1) |
...
### 1.1. Почта (коды входа) — smtp.bz, домен `spi-2015.ru`

Коды входа отправляются через `smtp.bz` от имени домена `spi-2015.ru`.
`backend/app/services/mail.py` пробует три способа по порядку (см. ADR-024):

1. **HTTP API smtp.bz** (`SMTP_API_KEY` заполнен) — `POST` на `api.smtp.bz` по
   HTTPS/443. **Приоритетный и рекомендуемый способ.**
2. **Сырой SMTP** (`SMTP_API_KEY` пуст, `SMTP_HOST` заполнен) — `aiosmtplib`
   (STARTTLS) по значениям `SMTP_*` из `.env`. Fallback для локальной разработки
   или провайдеров без HTTP API.
3. **Ничего не настроено** — код входа печатается в лог контейнера
   (`[DEV] Login code for ...`), письма не уходят (нормально для локальной проверки).

**Почему HTTP API приоритетнее.** Многие облачные VPS (в т.ч. Timeweb Cloud, на
котором проверялся этот деплой) по умолчанию блокируют исходящие SMTP-порты
**587 и 465** (анти-спам мера провайдера) — при этом приложение не падает, а
зависает на `POST /api/v1/auth/request-code` до таймаута, и кнопка «получить код»
внешне выглядит как ничего не делающая. Исходящий HTTPS (443) при этом
блокируется провайдерами исключительно редко, поэтому HTTP API — надёжный путь
по умолчанию на любом VPS. Если ваш провайдер не блокирует SMTP-порты, можно
обойтись только `SMTP_*` (шаг 2) без ключа API.

**Настройка через HTTP API (рекомендуется):**
1. Личный кабинет smtp.bz → https://smtp.bz/panel/user → скопировать API-ключ.
2. В `.env`: `SMTP_API_KEY=<ключ>`. Значения `SMTP_HOST/USER/PASSWORD` при этом
   игнорируются — можно оставить пустыми.
3. Домен `spi-2015.ru` должен быть подтверждён в кабинете smtp.bz (SPF/DKIM
...
4. `MAIL_FROM=noreply@spi-2015.ru` (любой адрес на подтверждённом домене).
5. Проверка после деплоя: `POST /api/v1/auth/request-code` с реальным e-mail — письмо
   должно прийти за секунды. Если нет — проверить ключ API и статус домена в
   кабинете smtp.bz, а не код приложения.

**Настройка через сырой SMTP (fallback, если HTTP API недоступен):**
1. Личный кабинет smtp.bz → раздел SMTP-авторизации → взять `SMTP_HOST`,
   `SMTP_USER` и `SMTP_PASSWORD` (логин/пароль **для SMTP**, не пароль от кабинета).
   `SMTP_PORT=587`.
2. Убедиться, что `SMTP_API_KEY` **пуст** — иначе он используется вместо SMTP.
3. Учитывать: если исходящий 587/465 у провайдера заблокирован, запрос зависнет
   до 10-секундного таймаута и вернёт `502 mail_send_failed` — это ожидаемое
   поведение, не баг приложения; решение — переключиться на HTTP API (см. выше)
   либо попросить провайдера снять блокировку портов.
...
# Почта (ключ API из кабинета smtp.bz — https://smtp.bz/panel/user, см. §1.1):
SMTP_API_KEY=<...>
...
| Код входа на e-mail не приходит | Проверить `SMTP_API_KEY` и статус домена `spi-2015.ru` (SPF/DKIM) в кабинете smtp.bz, см. §1.1; при пустых `SMTP_API_KEY`/`SMTP_HOST` код только в логе `api` |
| «Получить код» не работает / зависает, `502 mail_send_failed` | Обычно исходящий SMTP (587/465) заблокирован провайдером VPS — переключиться на `SMTP_API_KEY` (HTTP API, порт 443), см. §1.1 и ADR-024 |
